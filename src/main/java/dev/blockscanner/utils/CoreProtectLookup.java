...
public class CoreProtectLookup {
    ...
    public List<TraceResult> lookup(Block block) {
        ...
        processEntries(api.blockLookup(x, y, z, world, secondsBack), api, byPlayer, maxPerPlayer, false);

        // ── 2. Container session lookup (open/close + item movements) ─────────
        if (block.getState() instanceof Container) {
            processEntries(api.sessionLookup(x, y, z, world, secondsBack), api, byPlayer, maxPerPlayer, true);
        }
    ...
    public List<BlockAction> lookupPlayer(Block block, String playerName) {
        ...
        collectForPlayer(api.blockLookup(x, y, z, world, secondsBack), api, playerName, actions, false);

        if (block.getState() instanceof Container) {
            collectForPlayer(api.sessionLookup(x, y, z, world, secondsBack), api, playerName, actions, true);
        }
